...
//	public static int numOfClusters = 0;
	 ...
	 * @param treeType
	 */
	public HilbertRTree(int dimension, float fillFactor, int capacity,
			PageFile file, int treeType, int splitPolicy) {
		if (dimension <= 1) {
		...
		if (splitPolicy < 1) {
			throw new IllegalArgumentException(
					"The order of splitting policy is at least 1.");
		}
		...
		HilbertRTDataNode root = new HilbertRTDataNode(this, Constants.NIL, 0);
		file.writeNode(root);

	}

	public HilbertRTree(PageFile file) {
		if (file.tree != null) {
			throw new IllegalArgumentException(
					"PageFile already in use by another rtree instance.");
		}
		...
		if (file.treeType == -1) {
			throw new IllegalArgumentException(
					"PageFile is empty. Use some other HilbertRTree constructor.");
		}
	...
	public int insert(Rectangle rectangle, int page) {
		...
		if (rectangle.getHigh().getDimension() != getDimension()) {
			throw new IllegalArgumentException(
					"Rectangle dimension different than HilbertRTree dimension.");
		}

		HilbertRTNode root = file.readNode(0);

		HilbertRTDataNode leaf = root.chooseLeaf(rectangle);
		// HilbertRTDataNode leaf = root.chooseLeaf(rectangle,
		// rectangle.getHilbertValue());
	...
	public int insert(Rectangle rectangle, int page, int TAG) {
		...
		return leaf.insert(rectangle, page, TAG);
	}
	
	/**
	 * 1. Find the host leaf.<br>
	 * 2. Delete @rectangle from node L.<br>
	 * 3. if L underflows, borrow some entries from s cooperating siblings.<br>
	 * &nbsp;&nbsp;		if all the siblings are ready to underflow, merge s+1 to s nodes,<br>
	 * &nbsp;&nbsp;		adjust the resulting nodes.<br>
	 * 4. adjust MBR and LHV in parent levels.<br>
	 * &nbsp;&nbsp;		form a set S that contains L and its cooperating siblings(if underflow has occurred).<br>
	 * &nbsp;&nbsp;		invoke adjustTree(S).<br>
	 * 
	 * @param rectangle want to delete
	 * @param TAG only as a tag
	 * @return
	 */
	public int delete(Rectangle rectangle, int TAG) {
		if (rectangle == null) {
			throw new IllegalArgumentException("Rectangle cannot be null.");
		}
		...
		HilbertRTDataNode leaf = root.findLeaf(rectangle);

		if (leaf != null) {
			return leaf.delete(rectangle, TAG);
		} else {
			System.out.println("Rectangle = " + rectangle + " not found!!!");
		}
	 ...
	 * @return 所有遍历的结点集合
	 */
	public List<HilbertRTNode> traversePostOrder(HilbertRTNode root) {
		if (root == null)
			throw new IllegalArgumentException("Node cannot be null.");

		List<HilbertRTNode> list = new ArrayList<HilbertRTNode>();

		if (!root.isLeaf()) {
			for (int i = 0; i < root.usedSpace; i++) {
				List<HilbertRTNode> a = traversePostOrder(((HilbertRTDirNode) root)
						.getChild(i));
	 ...
	 */
	public Enumeration<HilbertRTNode> intersection(Rectangle rectangle) {
		class IntersectionEnum implements Enumeration<HilbertRTNode> {
			private List<HilbertRTNode> nodes;
			...
			@Override
			public HilbertRTNode nextElement() {
				if (!hasNext) {
					throw new NoSuchElementException("intersection");
				}

				HilbertRTNode node = nodes.get(index);
				index++;
	...
	/**
	 * 查询算法。始于根节点，依次向下遍历整棵索引树并检查所有的同查询范围相交的
	 * 结点，若在叶子结点层，则将所有同查询范围相交的空间对象作为查询结果提出。
	 * @param rectangle
	 * @param node
	 * @return
	 */
	public List<HilbertRTNode> query(Rectangle rectangle, HilbertRTNode node) {
		return intersection(rectangle, node);
	}
	
	/**
	 * 查询算法。查询所有与指定@rectangle相交的所有Rectangle
	 * @param rectangle
	 * @param node
	 * @return
	 */
	public List<Rectangle> queryAllRectangles(Rectangle rectangle, HilbertRTNode node) {
		return intersectionRectangles(rectangle, node);
	}
	
	private List<Rectangle> intersectionRectangles(Rectangle rectangle,
			HilbertRTNode node) {
		...
		if (rectangle.getDimension() != getDimension()) {
			throw new IllegalArgumentException(
					"Rectangle dimension is different than Rtree dimension.");
		}
		...
		if (node.isLeaf()) {
			if (node.getNodeRectangle().isIntersection(rectangle))
				for (Rectangle rec : node.datas)
					if (rec != null && rec.isIntersection(rectangle))
						list.add(rec);
		} else {
			for (int i = 0; i < node.usedSpace; i++) {
				if (node.datas[i].isIntersection(rectangle)) {
					// 递归调用
					List<Rectangle> nodes = intersectionRectangles(rectangle,
							((HilbertRTDirNode) node).getChild(i));
	 ...
	 */
	public List<HilbertRTNode> intersection(Rectangle rectangle,
			HilbertRTNode node) {
		if (rectangle == null || node == null) {
			throw new IllegalArgumentException("Arguments cannot be null.");
		}
		...
		if (node.isLeaf()) {
			...
			for (int i = 0; i < node.usedSpace; i++) {
				if (node.datas[i].isIntersection(rectangle)) {
					// 递归调用
					List<HilbertRTNode> nodes = intersection(rectangle,
							((HilbertRTDirNode) node).getChild(i));
					for (int j = 0; j < nodes.size(); j++) {
						list.add(nodes.get(j));
					}
...
//		}
	 ...
	 */
	public List<HilbertRTNode> intersection_All(Rectangle rectangle,
			HilbertRTNode node) {
		if (rectangle == null || node == null) {
			throw new IllegalArgumentException("Arguments cannot be null.");
		}
		...
		if (node.getNodeRectangle().isIntersection(rectangle)) {
			...
			if (!node.isLeaf()) {
				for (int i = 0; i < node.usedSpace; i++) {
					if (node.datas[i].isIntersection(rectangle)) {
						// 递归调用
						List<HilbertRTNode> nodes = intersection_All(rectangle,
								((HilbertRTDirNode) node).getChild(i));
						for (int j = 0; j < nodes.size(); j++) {
							list.add(nodes.get(j));
						}
	 ...
	 */
	public List<Data> intersection_Rectangles(Rectangle rectangle,
			HilbertRTNode node) {
		if (rectangle == null || node == null) {
			throw new IllegalArgumentException("Arguments cannot be null.");
		}
	...
	public Data[] nearestSearch(Point queryPoint, float range, int n) {
		if (n <= 0 || range < 0 || queryPoint == null)
			throw new IllegalArgumentException(
					"HilbertRTree.nearestSearch: Illegal arguments");
	 ...
	 * @return
	 */
	protected List<Data> nearestNeighborSearch(HilbertRTNode node,
			Point queryPoint, float nearest, boolean nearestIsFixed) {
		...
		if (node.isLeaf())// node为叶子结点则比较Point到每个条目的最小距离
		{
			for (int i = 0; i < node.usedSpace; i++) {
				...
				/* 1、生成分支列表
				 * 对每个最新访问的非叶结点首先计算出其中所有的MBR的MINDIST、MINMAXDIST
				 * 值，并将这些值排序后放入活动分支链表ABL(Active Branch List)中
				 */
			BranchList[] branchList = new BranchList[node.usedSpace];
			for (int i = 0; i < node.usedSpace; i++) {
				HilbertRTNode rtNode = ((HilbertRTDirNode) node).getChild(i);
				branchList[i] = new BranchList(
						rtNode, 
						rtNode.getNodeRectangle().getMinDist(queryPoint), 
						rtNode.getNodeRectangle().getMinMaxDist(queryPoint));
			}
...
//						+ branchList.length);
		...
		for (i = 0; i < last; i++) {
			...
			if (branchList[i].minMaxDist < branchList[last - 1].minDist) {
				for (int j = 0; j < last; j++) {
					if ((i != j) && (branchList[j].minDist > branchList[i].minMaxDist)) {
						last = j;
						break;
					}
				}
	 ...
	 * @return
	 */
	public List<HilbertRTNode> traverseByLevel() {
		HilbertRTNode root = file.readNode(0);

		if (root == null)
			throw new IllegalArgumentException("Node cannot be null.");

		List<HilbertRTNode> list = traverseByLevel(root);
	 ...
	 */
	public List<HilbertRTNode> traverseByLevel(HilbertRTNode root) {
		if (root == null)
			throw new IllegalArgumentException("Node cannot be null.");

		List<HilbertRTNode> ret = new ArrayList<HilbertRTNode>();
		List<HilbertRTNode> list = traversePostOrder(root);

		for (int i = getTreeLevel(); i >= 0; i--) {
			for (int j = 0; j < list.size(); j++) {
				HilbertRTNode n = list.get(j);
				if (n.getLevel() == i) {
					ret.add(n);
				}
			}
