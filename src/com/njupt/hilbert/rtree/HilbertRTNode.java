...
/**
 * @ClassName HilbertRTNode
 * @Description
 */
public abstract class HilbertRTNode implements INode, Serializable {
	
	private static final long serialVersionUID = -7206820922582460028L;

	/**
	 * 结点所在的树
	 */
	protected transient HilbertRTree tree;
...
//	protected int[] leftSiblings;
//	protected int[] rightSiblings;
...
//	protected int MIN;
	...
	protected HilbertRTNode(HilbertRTree tree, int parent, int pageNumber, int level) {
		this.parent = parent;
		this.tree = tree;
...
//		MIN = Math.round(tree.getNodeCapacity() * tree.getFillFactor());
		
//		int splitPolicy = tree.getSplitPolicy();
//		if (splitPolicy > 1) {
//			leftSiblings = new int[splitPolicy - 1];
//			rightSiblings = new int[splitPolicy - 1];
//			for (int i = 0; i < leftSiblings.length; i++) {
//				/* default pageNumber is -3 which indicates NULL */
//				leftSiblings[i] = Constants.NO_SIBLINGS;
//				rightSiblings[i] = Constants.NO_SIBLINGS;
//			}
//		}
	}
	...
	protected void addData(Rectangle rectangle, int page) {
		...
		if (LHV < rectangle.getHilbertValue()) {
     ...
     *
     */
	public void addData(HilbertRTNode node) {
	...
	protected void deleteData(int i) {
		if (i < 0 || i >= usedSpace)
			throw new RuntimeException("Array's index is out of bound.");
		if (datas[i + 1] != null) {
...
//		}
	...
	protected void deleteNode(int i) {
		tree.file.deletePage(branches[i]);
	...
	protected void condenseTree() {
		if (isRoot()) {
			...
			if (!isLeaf() && usedSpace == 1) {
				HilbertRTNode n = tree.file.readNode(branches[0]);
				...
				if (!n.isLeaf()) {
					for (int i = 0; i < n.usedSpace; i++) {
						HilbertRTNode m = ((HilbertRTDirNode)n).getChild(i);
						m.parent = 0;//?
...
//				list.add(this);// 之前已经把数据删除了
	 ...
	 */
	@Deprecated
	protected void condenseTree(List<HilbertRTNode> list) {
		if (isRoot()) {
			// 根节点只有一个条目了，即只有左孩子或者右孩子
			if (!isLeaf() && usedSpace == 1) {
				HilbertRTNode n = tree.file.readNode(branches[0]);
...
//			}
	...
	public String toString() {
		String s = "< Page: " + pageNumber + ", Level: " + level 
				+ ", UsedSpace: " + usedSpace + ", Parent: " + parent + ", LHV: " + LHV + " >\n";
	 ...
	 */
	public List<Rectangle> search(Rectangle region, HilbertRTNode root) {
		return null;
	 ...
	 * 
	 * @param Rectangle
	 * @return HilbertRTDataNode
	 */
	protected abstract HilbertRTDataNode chooseLeaf(Rectangle rectangle);
	
	/**
	 * Returns the leaf node in which to place a new rectangle @rectangle
	 * @param rectangle
	 * @param hilbertValue
	 * @return
	 */
	protected abstract HilbertRTDataNode chooseLeaf(Rectangle rectangle, long hilbertValue);
	...
	protected abstract HilbertRTDataNode findLeaf(Rectangle rectangle);
}
